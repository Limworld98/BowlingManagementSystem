	...
	public void receiveLaneEvent(LaneEvent le) {
		if (lane.isPartyAssigned()) {
			...
			for (int k = 0; k < numBowlers; k++) {
				...
				for (int i = 0; i < 21; i++) {
					int[] bowlerScore = (int[]) ((HashMap) le.getScore()).get(bowlers.get(k));
					int nowScore = bowlerScore[i];

					if (nowScore != -1) {
						if ((i % 2 == 0 || i == 19) && nowScore == 10) {
							ballLabel[k][i].setText("X");
						} else if (i>0 && (i%2) == 1 && ((nowScore + bowlerScore[i - 1]) == 10)) {
							ballLabel[k][i].setText("/");
						} else if (nowScore == -2) {
							ballLabel[k][i].setText("F");
						} else {
							ballLabel[k][i].setText(Integer.toString(nowScore));
						}
					}
				}
			}
